        ...
        const EDIT_LEAD_MODULE = {
            ...
            register: function() {
                ...
                EDIT_LEAD_MODULE.$offcanvas.on('hide.bs.offcanvas', function () {
                    replaceUrl("{{ route('admin.vltrading.index') }}");
                });
            ...
            prepareUi: function() {
                ...
                // making ajax request
                const url = '{{ route('admin.vltrading.show', ':id') }}'.replace(':id', EDIT_LEAD_MODULE.scamLeadId);
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if(res.success) {
                            ...
                            const formActionUrl = "{{ route('admin.vltrading.update', ':id') }}".replace(':id', scamLead.id);
                            ...
                            $form.find('textarea[name="customer_description"]').val(scamLead.customer_description);
                            ...
                            const editUrl = "{{ route('admin.vltrading.edit', ':id') }}".replace(':id', scamLead.id);
                            replaceUrl(editUrl);
